...
public class LoggingAspect {
	...
	public Object aroundMethod(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
		...
		try {
			...
			//if (logFlag) {
			//	//前置通知
			//	//logger.info(CommonUtil.appendSerialNumber(remoteParameter, String.format("方法[%s]开始", classMethod)));
			//	//if (null != remoteParameter.getParam()) {
			//	//	logger.info(CommonUtil.appendSerialNumber(remoteParameter, "参数：%s", remoteParameter.getParam().toString()));
			//	//}
			//}
			//result = proceedingJoinPoint.proceed(args);
			//if (logFlag) {
			//	//后置通知
			//	//logger.info(CommonUtil.appendSerialNumber(remoteParameter, String.format("方法[%s]结束", classMethod)));
			//}
